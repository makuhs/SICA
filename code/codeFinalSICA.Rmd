...
### Data Load
```{r}
data <- read.csv("KuhsSICA_dataframes/fullBiologByWellSICA.csv")
```
...
#Calculate Phenotype
metrics <- data %>%
  mutate(niche.width = rowSums(.[4:98]!=0),
         total.growth = rowSums(.[4:98]),
         growth.eff = total.growth/niche.width)%>%
  dplyr::select(Plot, SICA, Isolate, niche.width, total.growth, growth.eff)
...
metricsFinal <- merge(metricSICA, metricControl, by=c("Plot", "Isolate")) %>%
  group_by(SICA) %>%
...
c_class <- read.csv("KuhsSICA_dataframes/Biolog_Nutrient_List.csv")
...
nutChange <- merge(nutCat, control, by=c("Isolate", "Nutrient", "BiologWell", "Plot", "CarbonSource", "CarbonType", "Count", "NumType"))%>%
  rename(SICA = SICA.x,
         Control = SICA.y,
         ODs = OD.x,
         ODc = OD.y) %>%
...
# Escape Ratio 
escape <- read.csv("KuhsSICA_dataframes/escape_ratio.csv")
...
```{r}
#soil data
soilData <- read.csv("KuhsSICA_dataframes/SoilData.csv")%>%
  filter(LabPlot_ID == 3| LabPlot_ID == 4)
   # mutate(treat = case_when(
   # LabPlot_ID %in% c(1, 3, 5) ~"A",
   # T ~ "B"))
...
#t-test: 
t.test(NO3.N ~ LabPlot_ID, data=soilData)
...
```{r}
#MIC values
rawData <- read.csv("KuhsSICA_dataframes/MICSICA_data.csv")%>%
  filter(Value == "MIC")%>%
...
ggplot(nutODLim3combo, aes(Isolate, reorder(CarbonSource, mean), fill=nutCat))+
  geom_tile()+
...
ggplot(nutODLim4combo, aes(Isolate, reorder(CarbonSource, mean), fill=nutCat))+
  geom_tile()+
  facet_wrap(~SICA, nrow = 1, scales = "free_x")+
  scale_fill_manual(values = c("#EA725D","#F5BB62", "#85D1D1", "#5499C7", "grey99"),
                    labels = c("Suppressed Growth","Reduced Growth", "Increased Growth", "Induced Growth", "No Change"),
...
```
#### Figure S5
```{r, messages = F, warnings = F, fig.dim=c(3.5, 5.5)}
...
#LNI Change in Growth per Carbon Source
ggplot(heatChange3, aes(Isolate, reorder(CarbonSource, mean), fill=Change))+
  geom_tile()+
...
#HNI Change in Growth per Carbon Source
ggplot(heatChange4, aes(Isolate, reorder(CarbonSource, mean), fill=Change))+
  geom_tile()+
  facet_grid(CarbonType~SICA, scales = "free", space = "free")+
  scale_fill_gradientn(
    colours = colorRampPalette(c('#FD7446', '#FFFFFF', '#709AE1'))(100),
...
tree <- ape::read.tree("KuhsSICA_dataframes/reverse.newick")

# Reduce name structure
new_tipLabels <- read.csv("KuhsSICA_dataframes/labelTree.csv")
tree$tip.label <- new_tipLabels$name

# Build metadata and filter to assigned groups
treeMeta <- data.frame(
  isolate = tree$tip.label,
  group = case_when(
    str_detect(tree$tip.label, "3") ~ "LNI",
    str_detect(tree$tip.label, "4") ~ "HNI",
    TRUE ~ NA
  )) %>%
  filter(!is.na(group))
...
dist_matrix <- dist_matrix[treeMeta$isolate, treeMeta$isolate]
treeMeta <- treeMeta[match(rownames(dist_matrix), treeMeta$isolate), ]

#check distribution
bd <- betadisper(as.dist(dist_matrix), treeMeta$group)
permutest(bd)

# Run PERMANOVA
adonis_result <- adonis2(as.dist(dist_matrix) ~ group, 
                         data = treeMeta, 
                         permutations = 999)
print(adonis_result)

# Filter tree to only HNI/LNI isolates
tree_filtered <- keep.tip(tree, treeMeta$isolate)
tree_ultra <- force.ultrametric(tree_filtered)

# Filter control data and check overlap
intersect(metricControl$Isolate, tree_ultra$tip.label)

# Build named vectors
niche_width <- setNames(control_df$niche.width, control_df$Isolate)
total_growth <- setNames(control_df$total.growth, control_df$Isolate)

# Run Blomberg's K
k_nw <- phylosig(tree_ultra, niche_width, method = "K", test = TRUE, nsim = 999)
k_tg <- phylosig(tree_ultra, total_growth, method = "K", test = TRUE, nsim = 999)
